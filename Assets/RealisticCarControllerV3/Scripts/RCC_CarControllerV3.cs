...
public class RCC_CarControllerV3 : RCC_Core
{
    ...
    private void NOS()
    {
        ...
        if (boostInput >= .8f && throttleInput >= .8f && NoS > 5)
        {
            ...
            GameplayUIManager.Agent.NOSFill.fillAmount = NoS / 100f;
            GameplayUIManager.Agent.NOSFillJoyStick.fillAmount = NoS / 100f;
    ...
    private void ResetCar()
    {
        if (speed < 5 && !rigid.isKinematic)
        {
            ...
            if (transform.eulerAngles.z < 300 && transform.eulerAngles.z > 60)
            {
                ...
                if (resetTime > 3)
                {
                    GetComponent<VehicleBehaviour>().StartBlinking();
    ...
    public void ManualResetCar()
    {
        if (CheckManualResetCar())
        {
            GetComponent<VehicleBehaviour>().StartBlinking();
            transform.rotation = Quaternion.Euler(0f, transform.eulerAngles.y, 0f);
            transform.position = new Vector3(transform.position.x, transform.position.y + 3, transform.position.z);
    ...
    public bool CheckManualResetCar()
    {
        if (speed < 5 && !rigid.isKinematic)
            return true;
        return false;
            ...
            if (useCollisionAudio)
            {
                ...
                if (crashClips.Length > 0)
                {
                    ...
                    if (collision.GetContact(0).thisCollider.gameObject.transform != transform.parent)
                    {
                        ...
                        {
                            if(GetComponent<VehicleBehaviour>() != null)
                            GetComponent<VehicleBehaviour>().PlayHaptics();
                        }
