...
public class RCC_AICarController : MonoBehaviour
{
    ...
    public enum NavigationMode { FollowWaypoints, ChaseTarget, FollowTarget, TargetPostion }
    ...
    public int nextWaypointPassDistance = 10;
    ...
    void Navigation()
    {
        ...
        switch (navigationMode)
        {
            ...
            case NavigationMode.TargetPostion:
                ...
                // Next waypoint and its position.
                RCC_Waypoint currentWaypoint1 = waypointsContainer.waypoints[currentWaypointIndex];

                // Checks for the distance to next waypoint. If it is less than written value, then pass to next waypoint.
                float distanceToNextWaypoint1 = Vector3.Distance(transform.position, currentWaypoint1.transform.position);
                // Setting destination of the Navigator. 
                //if (!navigator.hasPath)
                //    navigator.SetDestination(waypointsContainer.waypoints[currentWaypointIndex].transform.position);

                if (distanceToNextWaypoint1 != 0 && distanceToNextWaypoint1 < nextWaypointPassDistance)
                {
                    currentWaypointIndex++;
                    totalWaypointPassed++;
                ...
                if (!reversingNow)
                {
                    ...
                    throttleInput = (distanceToNextWaypoint1 < (nextWaypointPassDistance * (carController.speed / 30f))) ? (Mathf.Clamp01(currentWaypoint1.targetSpeed - carController.speed)) : 1f;
                    throttleInput *= Mathf.Clamp01(Mathf.Lerp(10f, 0f, (carController.speed) / maximumSpeed));
                    brakeInput = (distanceToNextWaypoint1 < (nextWaypointPassDistance * (carController.speed / 30f))) ? (Mathf.Clamp01(carController.speed - currentWaypoint1.targetSpeed)) : 0f;
                    handbrakeInput = 0f;

                    Quaternion a = new Quaternion(transform.rotation.x,transform.rotation.y,transform.rotation.z, transform.rotation.w);

                    Vector3 relativePos = currentWaypoint1.transform.position - transform.position;
                    Quaternion rotation = Quaternion.LookRotation(relativePos, Vector3.up);
                    transform.rotation = Quaternion.Lerp(a, rotation, 0.05f);
