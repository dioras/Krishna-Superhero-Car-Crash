...
public class BCG_EnterExitManager : MonoBehaviour
{
    ...
    public void BCG_Player_OnBCGPlayerEnteredAVehicle(BCG_EnterExitPlayer player, BCG_EnterExitVehicle vehicle)
    {
      ...
      //  player.gameObject.SetActive(false);     //	Disable the actual player character.
        ...
        //TODO: Set player positon in car and set character as vehicle child of that car
        player.transform.GetComponent<ThirdPersonController>().enabled = false;
        player.transform.GetComponent<CharacterController>().enabled = false;

        player.transform.GetComponent<Animator>().Play("CarSitting");
        Transform sittingpos = vehicle.transform.Find("CarSittingPos").transform;
        player.transform.position = sittingpos.position;
        player.transform.rotation = sittingpos.rotation;
        player.transform.localScale = sittingpos.localScale;

        // player.transform.GetComponent<Animator>().Play("GetIn");
        ...
        GameManager.Agent.SetupVehicles(vehicle.gameObject);

        GameObject temp = GameplayUIManager.Agent.FollowCar;
        temp.GetComponent<FinishCamera>().Car = vehicle.transform;
        temp.SetActive(true);
        //GameplayUIManager.Agent.FollowCarCam.SetActive(true);
        GameplayUIManager.Agent.cineCam.GetComponent<Cinemachine.CinemachineVirtualCamera>().Follow = temp.transform;//temp.transform;
        GameplayUIManager.Agent.cineCam.SetActive(true);
        GameplayUIManager.Agent.cineCam.GetComponent<Cinemachine.CinemachineVirtualCamera>().LookAt = vehicle.transform;
    ...
    public void BCG_Player_OnBCGPlayerExitedFromAVehicle(BCG_EnterExitPlayer player, BCG_EnterExitVehicle vehicle)
    {
        ...
        player.transform.GetComponent<ThirdPersonController>().enabled = true;
        player.transform.GetComponent<CharacterController>().enabled = true;
        player.transform.GetComponent<Animator>().Play("Idle Walk Run Blend");
