...
public class BCG_EnterExitPlayer : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(rayPosition, rayDirection, out hit, 1.5f))
        {
            ...
            {
                if(hit.collider.transform.GetComponentInParent<VehicleBehaviour>()!= null)
                if(hit.collider.transform.GetComponentInParent<VehicleBehaviour>().isLocked == true)
                {
                    GameplayUIManager.Agent.Adicon.SetActive(true);
                }
                else
                {
                    GameplayUIManager.Agent.Adicon.SetActive(false);
                }
                showGui = true;
    ...
    private void OnGUI()
    {
        ...
        if (showGui)
        {
            ...
            GameplayUIManager.Agent.InteractButton.SetActive(true);
        }
        else
            GameplayUIManager.Agent.InteractButton.SetActive(false);
